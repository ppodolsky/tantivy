...
#[cfg(test)]
mod tests {
    use crate::query::{Scorer, BM25Weight};
    ...
    fn compute_checkpoints_for_each_pruning(term_scorers: Vec<TermScorer>, n: usize) -> Vec<(DocId, Score)> {
    ...
    fn test_block_wand_aux(posting_lists: &[Vec<(DocId, u32)>], fieldnorms: &[u32]) {
        ...
        let postings_lists_expanded: Vec<Vec<(DocId, u32)>> = posting_lists.iter()
            .map(|posting_list|
                posting_list
                    .into_iter()
                    .cloned()
                    .flat_map(|(doc, term_freq)|
                        (0 as u32..REPEAT as u32)
                            .map(move |offset| (doc * (REPEAT as u32) + offset, if offset == 0 { term_freq } else { 1 }))
                    )
                    .collect::<Vec<(DocId, u32)>>()
            )
            .collect::<Vec<_>>();

        let total_fieldnorms: u64 = fieldnorms_expanded.iter().cloned().map(|fieldnorm| fieldnorm as u64).sum();
        let average_fieldnorm = (total_fieldnorms as Score) / (fieldnorms_expanded.len() as Score);
        let max_doc = fieldnorms_expanded.len();

        let term_scorers: Vec<TermScorer> = postings_lists_expanded.iter()
            .map(|postings| {
                let bm25_weight = BM25Weight::for_one_term(postings.len() as u64, max_doc as u64, average_fieldnorm);
                TermScorer::create_for_test(postings, &fieldnorms_expanded[..], bm25_weight)
            })
            .collect();
        let checkpoints_for_each_pruning  = compute_checkpoints_for_each_pruning(term_scorers.clone(), 3);
        let checkpoints_manual = compute_checkpoints_manual(term_scorers, 3);
        assert_eq!(checkpoints_for_each_pruning.len(), checkpoints_manual.len());
        for (&(left_doc, left_score), &(right_doc, right_score)) in checkpoints_for_each_pruning.iter().zip(checkpoints_manual.iter()) {
    ...
    #[test]
    fn test_reproduce() {
        test_block_wand_aux(&[vec![(0, 1)], vec![(0, 2)], vec![(1, 8)]], &[472, 2]);
    }
}
